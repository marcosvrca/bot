...
export function startGoogleReminderWorker(params: {
  prisma: PrismaClient;
  outbound: OutboundService;
  logger: Logger;
  intervalMs?: number;
}): { stop: () => void } {
  const repo = new GoogleAppointmentRepository(params.prisma);
    ...
    try {
      ...
      for (const appointment of due) {
        ...
        params.logger.info(
          ...
          "google-reminder.sent",
        );
      }
    } catch (err) {
      params.logger.error({ err }, "google-reminder.worker.error");
    } finally {
      running = false;
    }
  };
